...
const ViewBook = () => {
    ...
    const fetchDetails = async () => {
      try {
        ...
        const message =
          ...
          err.message ||
          "Failed to load book specifications.";
        toast.error(message);
      } finally {
        setLoading(false);
      }
  ...
  if (loading)
    return (
      <div className="flex flex-col items-center justify-center py-24 min-h-[60vh]">
        ...
        <p className="mt-4 text-[10px] font-bold text-text-secondary uppercase tracking-[0.2em]">
          Retaining book records...
        </p>
      </div>
    );
  ...
  const epubDownloadUrl = hasEpub
    ? bookApi.getReadBookUrl(book.id || book._id)
    : null;
  ...
  const audioChapters = Array.isArray(book.audiobooks)
    ? book.audiobooks
    ...
    <div className="flex flex-col gap-8 pb-16 animate-fade-in text-left font-['Outfit'] max-w-[1400px] mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-text-secondary text-xs font-semibold opacity-60">
            Master Record ID: {book.id || book._id}
          </p>
        </div>
        <div className="flex gap-3">
          <Button variant="secondary" icon={ShoppingBag} size="sm">
            View on Store
          </Button>
          <Button
            icon={Pencil}
            ...
            <div className="relative z-10 space-y-6">
              <div className="aspect-[3/4] w-full max-w-[240px] mx-auto rounded-xl overflow-hidden shadow-2xl border border-border/50 transition-all duration-500 group-hover:scale-[1.03]">
                ...
                <div className="absolute top-3 left-3 flex flex-col gap-1.5">
                  {book.is_bestselling && (
                    <span className="px-2 py-1 bg-emerald-500 text-white text-[8px] font-black uppercase tracking-tighter rounded shadow-lg">
                      Bestseller
                    </span>
                  )}
                  {book.is_trending && (
                    <span className="px-2 py-1 bg-blue-600 text-white text-[8px] font-black uppercase tracking-tighter rounded shadow-lg">
                      Trending
                    </span>
                  )}
                </div>
                {book.is_premium && (
                  <div className="absolute top-3 right-3 bg-amber-500 text-white px-2 py-1 rounded text-[8px] font-black uppercase shadow-lg flex items-center gap-1">
              ...
              <div className="pt-6 border-t border-border/50">
                <div className="grid grid-cols-2 gap-6">
                  <div>
                    ...
                    {book.original_price && (
                      <p className="text-xs text-text-secondary line-through opacity-50">
                        ₹{book.original_price}
                      </p>
                    )}
                  </div>
                  <div className="text-right">
                    ...
                    <div className="flex items-center justify-end gap-2">
                      <span
                        className={`h-2 w-2 rounded-full ${book.stock > 10 ? "bg-emerald-500" : "bg-amber-500 animate-pulse"}`}
                      ></span>
          ...
          <div className="bg-surface border border-border rounded-2xl p-6 shadow-sm space-y-4">
            ...
            <div className="grid grid-cols-1 gap-3">
              {[
                {
                  label: "ISBN-13",
                  val: book.isbn || "978-XXXXXXXXXX",
                  icon: Hash,
                },
                {
                  label: "Edition Lang",
                  val: book.language || "English",
                  icon: Globe,
                },
                {
                  label: "Category",
                  val: book.category?.name || "General Arch",
                  icon: Tag,
                },
                {
                  label: "Release Date",
                  val: book.published_date || "N/A",
                  icon: Calendar,
                },
                {
        ...
        <div className="lg:col-span-2 space-y-8">
          <div className="bg-surface border border-border rounded-2xl p-8 shadow-sm space-y-8">
            <div>
              ...
              <div className="flex items-center gap-3">
                ...
                <p className="text-sm font-bold text-text-secondary flex items-center gap-2 tracking-tight">
                  <User size={16} className="text-primary" /> Authority:{" "}
                  {book.author}
                </p>
              </div>
            </div>
              ...
              <div className="space-y-4">
                ...
                <div
                  ...
                  dangerouslySetInnerHTML={{
                    __html:
                      book.description ||
                      "No extensive description data available for this edition.",
                  }}
                />
              </div>
              {book.highlights && (
                <div className="space-y-4">
                  <h4 className="text-[11px] font-black text-text-primary uppercase tracking-[0.2em] flex items-center gap-2">
                    <ListChecks size={14} className="text-primary" /> Key
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div
                className={`p-6 rounded-2xl border-2 transition-all ${hasPdf ? "border-primary/20 bg-primary/5 group cursor-pointer hover:bg-primary/10" : "border-border bg-background opacity-50grayscale"}`}
              >
                <div className="flex items-center justify-between mb-4">
                  <div
                    className={`p-3 rounded-xl ${hasPdf ? "bg-primary text-white" : "bg-border text-text-secondary"}`}
                  >
                    <FileText size={20} />
                  </div>
                  <span
                    className={`text-[9px] font-black uppercase tracking-widest ${hasPdf ? "text-primary" : "text-text-secondary"}`}
                  >
                ...
                <p className="text-[10px] font-bold text-text-secondary uppercase mb-4">
                  Full Digital Text Access
                </p>
                <Button
                  fullWidth
                  size="sm"
              ...
              <div
                className={`p-6 rounded-2xl border-2 transition-all ${hasEpub ? "border-amber-500/20 bg-amber-500/5 group cursor-pointer hover:bg-amber-500/10" : "border-border bg-background opacity-50 grayscale"}`}
              >
                <div className="flex items-center justify-between mb-4">
                  <div
                    className={`p-3 rounded-xl ${hasEpub ? "bg-amber-500 text-white" : "bg-border text-text-secondary"}`}
                  >
                    <Layers size={20} />
                  </div>
                  <span
                    className={`text-[9px] font-black uppercase tracking-widest ${hasEpub ? "text-amber-600" : "text-text-secondary"}`}
                  >
            ...
            {audioChapters.length > 0 && (
              <div className="space-y-6 pt-6 border-t border-border/50">
                <div className="flex items-center justify-between">
                  <h4 className="text-[11px] font-black text-text-primary uppercase tracking-[0.2em] flex items-center gap-2">
                    <Volume2 size={16} className="text-primary" /> Audio Edition
                    Content
                  </h4>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
                  {audioChapters.map((ch, idx) => (
                    <div
                      key={idx}
                      className="flex items-center gap-4 p-4 rounded-xl border border-border bg-background/40 hover:border-primary/40 transition-colors group"
                    >
                      ...
                      <div className="flex-1 overflow-hidden">
                        ...
                        <p className="text-[9px] font-bold text-text-secondary opacity-50">
                          Narrated Audio Track
                        </p>
                      </div>
                      <Volume2
                        size={16}
                        className="text-text-secondary group-hover:text-primary transition-colors cursor-pointer"
                      />
                    </div>
                  ))}
                </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {coverUrl && (
              <div className="bg-surface border border-border rounded-2xl p-6 shadow-sm space-y-4">
                <h4 className="text-[10px] font-black text-text-primary uppercase tracking-[0.15em] flex items-center gap-2">
                  <ImageIcon size={14} className="text-primary" /> Marketing
                  Cover
                </h4>
            ...
            {gallery.length > 0 && (
              <div
                className={`bg-surface border border-border rounded-2xl p-6 shadow-sm space-y-4 ${!coverUrl ? "md:col-span-2" : ""}`}
              >
                <h4 className="text-[10px] font-black text-text-primary uppercase tracking-[0.15em] flex items-center gap-2">
                  <Layers size={14} className="text-primary" /> Gallery Archive{" "}
                  {gallery.length > 1 && `(${gallery.length})`}
                </h4>
                <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-2 lg:grid-cols-4 gap-3">
                  {gallery.map((img, idx) => (
                    <div
                      key={idx}
                      className="aspect-square rounded-lg overflow-hidden border border-border bg-background group hover:border-primary transition-all cursor-pointer shadow-sm"
                    >
                      <img
